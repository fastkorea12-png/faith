const stages = [
  ...
  },
  {
    ...
    code: "BETTER-04",
    minutes: 20,
    hint1: "방 안 4개 구역(문, 창가, 바닥, 선반)의 갈림길 일지를 탐색해 순례자가 내린 참된 결단 증언을 대조하게 합니다.",
    hint2: "여정 지도 매트에 문장(돌아갈 수 있었지만→그 길을 떠나→더 나은 본향을→사모하였다) 순서대로 4개 카드를 연결하여 8구간 궤적(오→위→왼→위→오→위→왼→위)을 확인하게 합니다.",
    hostNote: "10인실 방 1팀씩 순차 진행(18~22분). 8자리 방향 자물쇠: 오른쪽→위→왼쪽→위→오른쪽→위→왼쪽→위. 이전 팀 퇴장 후 카드 4장 봉투 복원 확인.",
  },
  {
    id: "home",
    step: "05",
